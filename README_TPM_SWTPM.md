...
Load module:
```bash
sudo insmod ~/vtpm_proxy/tpm_vtpm_proxy.ko
```
...
Build and run:
```bash
gcc -o /tmp/vtpm_bridge3 /tmp/vtpm_bridge3.c
sudo /tmp/vtpm_bridge3
```
...
To run crosvm on the host:
```bash
sudo ./crosvm run --mem 4096 --cpus 2 --block /home/z3r0cool/vm/ubuntu-crosvm/noble-server-cloudimg-amd64.raw --block /home/z3r0cool/vm/ubuntu-crosvm/seed.iso --net tap-name=crosvm0 --swtpm-socket /tmp/swtpm.sock --disable-sandbox   --params "root=/dev/vda1 rw console=ttyS0 rootwait rootfstype=ext4 systemd.unit=multi-user.target" /mnt/vm-boot/vmlinuz-6.8.0-117-generic
